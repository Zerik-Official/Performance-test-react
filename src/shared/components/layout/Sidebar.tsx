  ...
  return (
    <>
      <header className="sticky top-0 z-40 flex h-14 items-center justify-between border-b bg-black px-4 text-white md:hidden">
        ...
        <button onClick={() => setOpen(!open)} className="rounded-md border border-white/20 p-2">
          <i className={cn(open ? 'fa-solid fa-xmark' : 'fa-solid fa-bars')} />
        </button>
      </header>
      <aside className={cn('fixed inset-y-0 left-0 z-30 w-64 border-r bg-black p-4 text-white transition-transform md:translate-x-0', open ? 'translate-x-0' : '-translate-x-full md:translate-x-0')}>
        ...
        <nav className="space-y-6">
          ...
          {user?.role === 'admin' && (
            <div className="space-y-1">
              ...
              {navAdmin.map((item) => (
                <Link key={item.href} to={item.href} className={cn('flex items-center gap-3 rounded-md px-3 py-2 text-sm hover:bg-white/10', isActive(item.href) && 'bg-white text-black')}>
                  <i className={item.icon} /> {item.label}
                </Link>
              ))}
            </div>
        ...
        <div className="absolute bottom-4 left-4 right-4">
          <button onClick={() => logout()} className="flex w-full items-center justify-center gap-2 rounded-md bg-white px-3 py-2 text-sm font-medium text-black hover:bg-white/90 cursor-pointer">
